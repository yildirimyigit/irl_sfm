...
OBS = np.array([rospy.get_param('/obstacle/position/x', 0.0), rospy.get_param('/obstacle/position/y', 0.0)]) # obstacle pose
SOURCE = np.array([rospy.get_param('/start/position/x', 0.0), rospy.get_param('/start/position/y', 0.0)])
...
class Recorder:
    ...
    def save(self):
        filename = str(SOURCE[0]) + '_' + str(GOAL[0]) + '_' + str(OBS[0]) + '_' + str(OBS[1]) + '.npy'
