...
class Recorder:
    def __init__(self):
        ...
        self.OBS = np.array(
            [rospy.get_param('obstacle/position/x', 0.0), rospy.get_param('obstacle/position/y', 0.0)])  # obstacle pose
    ...
    def pose_callback(self, msg):
        if msg.pose.position.y >= self.SOURCE[1]:
            ...
            # state = np.array([d_g_x, d_g_y, d_o_x, d_o_y, vx, vy, pose_x, pose_y, self.last_velo_points], dtype="object")
            state = np.array([d_g_x, d_g_y, d_o_x, d_o_y, vx, vy, pose_x, pose_y])
            self.demonstration.append(state)

    def save(self):
        filename = str(self.SOURCE[0]) + '_' + str(self.GOAL[0]) + '_' + str(self.OBS[0]) + '_' + str(
            self.OBS[1]) + '.npy'
