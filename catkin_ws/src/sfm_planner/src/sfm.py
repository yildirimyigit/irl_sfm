...
class SFMController:
    def __init__(self):
        ...
        self.starting_pose = PoseStamped(Header(0, 0, 'odom'), Pose(
            Point(rospy.get_param('start/position/x', 0.0), rospy.get_param('start/position/y', 0.0), 0),
            Quaternion(0, 0, rospy.get_param('start/orientation/z', 0.706),
                       rospy.get_param('start/orientation/w', 0.707))))
        self.goal_pose = PoseStamped(Header(0, 0, 'odom'), Pose(
            Point(rospy.get_param('goal/position/x', 0.0), rospy.get_param('goal/position/y', 13.0), 0),
            Quaternion(0, 0, rospy.get_param('goal/orientation/z', 0.706),
                       rospy.get_param('goal/orientation/w', 0.707))))
    ...
    def calculate_repulsive_force(self, scan):
        ...
        force = Vector3()
        force.x = self.force_strength * np.exp(
            (self.sum_radii - self.distance_magnitude_to_closest_obstacle) / self.force_range) * direction.x
        force.y = self.force_strength * np.exp(
            (self.sum_radii - self.distance_magnitude_to_closest_obstacle) / self.force_range) * direction.y
        ...
        while not rospy.is_shutdown() and publish:
            ...
            if self.distance.x < self.x_thr and self.distance.y < self.y_thr:  # goal reached
                ...
                if tf_ready:
                    ...
                    # rotate v by rot
                    unit_v = tf.transformations.unit_vector(
                        np.array([self.last_cmd_vel.linear.x, self.last_cmd_vel.linear.y, 0]))
                    q = list(unit_v)
                    q.append(0.0)
                    rot_v = tf.transformations.quaternion_multiply(tf.transformations.quaternion_multiply(rot, q),
                                                                   tf.transformations.quaternion_conjugate(rot))[:3]
